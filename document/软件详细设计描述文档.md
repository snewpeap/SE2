  ...
  - [4 结构视角](#4-%E7%BB%93%E6%9E%84%E8%A7%86%E8%A7%92)
    - [4.1 业务逻辑层的分解](#41-%E4%B8%9A%E5%8A%A1%E9%80%BB%E8%BE%91%E5%B1%82%E7%9A%84%E5%88%86%E8%A7%A3)
      ...
      - [4.1.3 cinemabl模块](#413-cinemabl%E6%A8%A1%E5%9D%97)
        ...
        - [4.1.3.5 业务逻辑层的设计原理](#4135-%E4%B8%9A%E5%8A%A1%E9%80%BB%E8%BE%91%E5%B1%82%E7%9A%84%E8%AE%BE%E8%AE%A1%E5%8E%9F%E7%90%86)
...
![userbl模块各个类的设计](https://github.com/Translucentwall/UMLImage/blob/master/docTask4/userbl_detailed_class_diagram.png?raw=true)
...
|```TicketsMapper.updateByPrimaryKeySelective(Ticket record)```|todo|
...
#### 4.2.4 vipcarddataservice 模块

##### 4.2.4.1 模块概述

vipcarddataservice模块的职责参见体系结构文档。

##### 4.2.4.2 模块内部类的接口规范

- 表21 VIPCardDataService模块的接口规范

| 提供的服务（供接口）|||
| - | - | - |
| VipcardDataService.getVIPReductionStrategy | 语法| ```public List<VIPReductionStrategyPO> getVIPReductionStrategy(double targetAmount) ``` |
|| 前置条件 | 无|
|| 后置条件 | 返回一个包含所有VIPReductionStrategyPO的列表|
| VipcardDataService.findByUserID| 语法| ```public VipcardPO findByUserID(VIPcardPO po) ```|
|| 前置条件 | 无|
|| 后置条件 | 按用户名ID进行查找返回相应的 VipcardPO结果|
| VipcardDataService.insert| 语法| ```public void insert(VipcardPO po) ```|
|| 前置条件 | 同样ID的po记录在数据库中不存在|
|| 后置条件 | 在数据库中增加一个po记录|
| VipcardDataService.update| 语法| ```public void update(VipcardPO po) ```|
|| 前置条件 | 同样ID的po记录在数据库中存在|
|| 后置条件 | 更新一个po记录|
| VipcardDataService.init| 语法| ```public void init() ```|
|| 前置条件 | 无|
|| 后置条件 | 初始化持久化数据库|
| VipcardDataService.finish| 语法| ```public void finish() ```|
|| 前置条件 | 无|
|| 后置条件 | 结束持久化数据库的使用|
